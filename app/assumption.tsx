...
export default function Assumption() {
  ...
  const [assumptionInput, setAssumptionInput] = useState(session.assumption ?? "");
  const [error, setError] = useState<string | null>(null);

  const onNext = () => {
    const value = assumptionInput.trim();

    if (value.length === 0) {
      setError("Assumption required.");
      return;
    }

    if (value.length > 180) {
      setError("180 character limit exceeded.");
      return;
    }
    ...
    >
      ...
      >
        <View style={styles.content}>
          ...
          <Text style={styles.title}>Assumption</Text>

          <TextInput
            value={assumptionInput}
            onChangeText={setAssumptionInput}
            multiline
            maxLength={180}
            style={styles.input}
